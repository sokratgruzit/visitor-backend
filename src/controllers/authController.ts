...
export const registerUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const user = await prisma.user.create({
            data: { email, password: hashed, name, isActive: false },
        });
...
export const refreshToken = async (req: Request, res: Response) => {
    ...
    try {
        ...
        return res
        .status(401)
        .json({ success: false, message: "Просроченный или неверный refresh токен" });
    }
};
...
export const verifyEmail = async (req: Request, res: Response) => {
    ...
    if (!token) return res.redirect(`${baseUrl}/email-confirmed?msg=empty`); 
...
export const checkAccess = async (req: AuthRequest, res: Response) => {
    try {
        const user = req.user; // предполагается, что authMiddleware добавил req.user
...
export const resendEmail = async (req: AuthRequest, res: Response) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Ошибка в отправке почты",
        });
    }
};
